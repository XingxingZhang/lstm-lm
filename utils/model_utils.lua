...
function model_utils.combine_fflstm_parameters( fflstm )
    ...
    local function getFFLSTMParameters(fflstm, parameters, gradParameters)
      ...
      for _, node in ipairs(fflstm.forwardnodes) do
        ...
        if node.data.module then
          if node.data.annotations.name ~= 'ngram_lookup' then
            local mp,mgp = node.data.module:parameters()
            if mp and mgp then
              for i = 1,#mp do
                table.insert(parameters, mp[i])
    ...
    getFFLSTMParameters(fflstm, parameters, gradParameters)
  ...
  for _, node in ipairs(fflstm.forwardnodes) do
    if node.data.module then
      if node.data.annotations.name == 'ngram_lookup' then
        ...
        print('ngram_lookup found!')
      elseif node.data.annotations.name == 'lstm_lookup' then
        lstm_lookup = node.data.module
        print('lstm_lookup found!')
      end
    end
  end
